...
/// flutter_tts 기반 — Android 시스템 id-ID / ms-MY voice 사용.
///
/// 화자별 음성: 기기에 남/여 보이스가 있으면 voice 전환,
/// 없으면 피치(남 0.72 / 여 1.12)로 구분한다.
/// [variant] 를 바꾸면 인도네시아어 ↔ 말레이어 로케일이 전환된다.
class TtsService {
  TtsService._();
  ...
  String? _speaking;
  LangVariant _variant = LangVariant.id;

  final Map<LangVariant, Map<String, String>?> _maleVoice = {};
  final Map<LangVariant, Map<String, String>?> _femaleVoice = {};
  bool _voicesScanned = false;

  LangVariant get variant => _variant;

  Future<void> setVariant(LangVariant v) async {
    if (_variant == v) return;
    _variant = v;
    if (_initialized) {
      await _tts.setLanguage(v.ttsLocale);
    }
  }

  Future<void> _ensureInit() async {
    if (_initialized) return;
    await _tts.setLanguage(_variant.ttsLocale);
  ...
  /// id/ms 보이스 중 이름에 male/female 힌트가 있는 것을 1회 스캔.
  Future<void> _scanVoices() async {
    ...
    try {
      ...
      for (final v in voices) {
        ...
        final LangVariant? lv = locale.startsWith('id')
            ? LangVariant.id
        ...
        final voice = {'name': name, 'locale': (v['locale'] ?? '').toString()};
        if (_maleVoice[lv] == null &&
            lower.contains('male') &&
            !lower.contains('female')) {
          _maleVoice[lv] = voice;
        }
        if (_femaleVoice[lv] == null && lower.contains('female')) {
          _femaleVoice[lv] = voice;
        }
      }
    } catch (_) {
  ...
  Future<void> speakAs(String text, {required String gender}) async {
    ...
    final voice = male ? _maleVoice[_variant] : _femaleVoice[_variant];
